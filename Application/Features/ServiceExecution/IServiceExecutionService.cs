...
public interface IServiceExecutionService
{
    ...
    Task<Result<ServiceExecutionResultDto>> UpdateWorkPerformedAsync(int orderServiceId, int currentPersonId, int currentUserId, IReadOnlyList<string> currentRoles, UpdateWorkPerformedRequest request, CancellationToken cancellationToken = default);
    Task<Result<ServiceExecutionResultDto>> AssignMechanicAsync(int orderServiceId, AssignMechanicRequest request, int currentUserId, CancellationToken cancellationToken = default);
    Task<Result<ServiceExecutionResultDto>> UnassignMechanicAsync(int orderServiceId, UnassignMechanicRequest request, int currentUserId, CancellationToken cancellationToken = default);
    Task<Result<ServiceExecutionResultDto>> RequestPartAsync(int orderServiceId, int currentPersonId, int currentUserId, IReadOnlyList<string> currentRoles, RequestOrderServicePartRequest request, CancellationToken cancellationToken = default);
    ...
    Task<Result<ServiceExecutionResultDto>> ChangeOrderServicePartQuantityAsync(int orderServicePartId, int currentPersonId, int currentUserId, IReadOnlyList<string> currentRoles, ChangeOrderServicePartQuantityRequest request, CancellationToken cancellationToken = default);
